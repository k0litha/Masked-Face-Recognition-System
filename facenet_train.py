...
class Facenet():
    def __init__(self,para_dict):
        ...
        train_img_dir = para_dict['train_img_dir']# dataset CASIA
        test_img_dir = para_dict['test_img_dir']# dataset LFW
        label_dict = para_dict['label_dict']


        # read training set labels and paths 
        if isinstance(train_img_dir, list):
            for idx, img_dir in enumerate(train_img_dir):
                if idx == 0:
                    # convert the lablel name to number dictonary
    ...
    def model_init(self,para_dict):
        ...
        #inference selection(inception V1  used)
...
if __name__ == "__main__":
    ...
    train_img_dir = [r"C:\faceRec\dataset\casia_aligned", r"C:\faceRec\dataset\casia-alignedmask"] #CASIA masked and unmasked face dataset folders
    test_img_dir = r"C:\faceRec\dataset\lfw"#LFW dataset
    label_dict = None
    embed_length = 128
    ...
    model_shape = [None, 112, 112, 3]  # at least[None,80,80,3] for inception_resnet_v1
    infer_method = 'inception_resnet_v1'  # 'simple_resnet' or "inception_resnet_v1" . for our project wehave used inception v1
    loss_method = "cross_entropy"
    opti_method = "adam"
    learning_rate = 5e-4
    save_dir = r"C:\faceRec\models\model_saver"
    ...
    batch_size = 32  # tested recomendation is  <= 96 if 6GB GPU
    ratio = 0.4  # <1.0
